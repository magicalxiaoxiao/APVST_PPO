...
# @torch.jit.script
def ssim(X, Y, window, data_range: float, use_padding: bool = False, weight_tensor=None):
    '''
    ...
    if weight_tensor is None:
        ...
        ssim_val = (ssim_map * weight_tensor).sum() / weight_tensor.sum()  # reduce along CHW
        cs = (cs_map * weight_tensor).sum() / weight_tensor.sum()
...
class SSIM(torch.nn.Module):
    ...
    def __init__(self, window_size=11, window_sigma=1.5, data_range=1., channel=3, use_padding=True):
        '''
        :param window_size: the size of gauss kernel
    ...
    def forward(self, X, Y, weight_tensor=None):
        if weight_tensor is None:
            r = ssim(X, Y, window=self.window, data_range=self.data_range, use_padding=self.use_padding)  
        else: 
            r = ssim(X, Y, window=self.window, data_range=self.data_range, use_padding=self.use_padding, weight_tensor=weight_tensor)
        return r[0]
...
class MSE(torch.nn.Module):
    ...
    def forward(self, X, Y, weight_tensor=None):
        if weight_tensor is None:
            return torch.mean(self.squared_difference(255 * X, 255 * Y))   # / 255.
        else:
            return (self.squared_difference(255 * X, 255 * Y) * weight_tensor).sum() / weight_tensor.sum()


class Distortion(torch.nn.Module):
    def __init__(self, distortion_metric):
    ...
    def forward(self, X, Y, weight_tensor=None):
        return self.dist.forward(X, Y, weight_tensor).mean()  # / 255.
